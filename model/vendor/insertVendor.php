...
if (isset($_POST['vendorDetailsStatus'])) {
	...
	if (isset($fullName) && isset($mobile)) {
		...
		$sql = 'INSERT INTO vendor(fullName, email, mobile, address, district, status) VALUES(:fullName, :email, :mobile, :address, :district, :status)';
		$stmt = $conn->prepare($sql);
		$stmt->execute(['fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'address' => $address, 'district' => $district, 'status' => $status]);
