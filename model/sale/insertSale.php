...
if (isset($_POST['saleDetailsItemNumber'])) {
	...
	if (!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)) {
		...
		if ($customerID == '') {
			echo '<div class="alert alert-danger">Please enter a Customer ID.</div>';
			exit();
		}
		...
		if (filter_var($customerID, FILTER_VALIDATE_INT) === 0 || filter_var($customerID, FILTER_VALIDATE_INT)) {
			...
			echo '<div class="alert alert-danger">Please enter a valid Customer ID</div>';
			exit();
		}

		// Check if itemNumber is empty
		if ($itemNumber == '') {
			echo '<div class="alert alert-danger">Please enter Item Number.</div>';
			exit();
		}

		// Check if unit price is empty
		if ($unitPrice == '') {
			echo '<div class="alert alert-danger">Please enter Unit Price.</div>';
			exit();
		}
		...
		if ($stockStatement->rowCount() > 0) {
			...
			if ($currentQuantityInItemsTable <= 0) {
				...
				echo '<div class="alert alert-danger">Stock is empty. Therefore, can\'t make a sale. Please select a different item.</div>';
				exit();
			} elseif ($currentQuantityInItemsTable < $quantity) {
				// Requested sale quantity is higher than available item quantity. Hence abort 
				echo '<div class="alert alert-danger">Not enough stock available for this sale. Therefore, can\'t make a sale. Please select a different item.</div>';
				exit();
			} else {
				...
				if ($customerStatement->rowCount() > 0) {
					...
					echo '<div class="alert alert-success">Sale details added to DB and stocks updated.</div>';
					exit();
				} else {
			...
			echo '<div class="alert alert-success">Item does not exist in DB.</div>';
			exit();
		}
	} else {
