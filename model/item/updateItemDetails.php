...
if (isset($_POST['itemNumber'])) {
	...
	if (!empty($itemNumber) && !empty($itemName) && isset($itemDetailsQuantity) && isset($itemDetailsUnitPrice)) {
		...
		// insert purchase record if quantity and unit price are provided
		if ($itemDetailsQuantity > 0 && $itemDetailsUnitPrice > 0) {
			$today = date('Y-m-d');
			$vendorID = 1;
			$vendorName = 'System Default';
			...
			$insertPurchaseStatement->execute(['itemNumber' => $itemNumber, 'purchaseDate' => $today, 'itemName' => $itemName, 'unitPrice' => $itemDetailsUnitPrice, 'quantity' => $itemDetailsQuantity, 'vendorName' => $vendorName, 'vendorID' => $vendorID]);
		}
